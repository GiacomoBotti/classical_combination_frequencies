...
! Condizioni iniziali e parametri vengono letti

      open(unit=101,file="input",status='old')
      read(101,*) !Condizioni iniziali
      read(101,*) nu
      read(101,*) !Passo temporale
      read(101,*) dt
      read(101,*) !Numero di passi
      read(101,*) Nsteps
      read(101,*) !Parametri
      read(101,*) omega,mass,zpe_scaling
      close(101)
! Definisco a negativa per avere il plateau a dx
! e la trattazione corretta
      De=38293.d0/4400.d0
...
! Dinamica con Velocity Verlet 
      ...
      time=0.d0
      qdot = qdot0
      call gradiente(omega,a,De,q,qdot,qddot)
      call potenziale(omega,a,De,q,qdot,V)
      E = (qdot**2)/2.d0 + V
      write(666,*) time, E
      write(777,*) q, qdot
      do i = 1,Nsteps
...
!**********************************************************************
      ...
      subroutine potenziale(omega,a,De,q,qdot,V)
      ...
      end subroutine potenziale

!**********************************************************************
   
      subroutine gradiente(omega,a,De,q,qdot,qddot)
